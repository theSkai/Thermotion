...
{
	double ret1 = m_elements[0] > 1?1:m_elements[0] < 0?0:m_elements[0];
	double ret2 = m_elements[1] > 1?1:m_elements[1] < 0?0:m_elements[1];
	double ret3 = m_elements[2] > 1?1:m_elements[2] < 0?0:m_elements[2];
	return Vector3f
		(
			ret1, ret2, ret3
		);
...
void Vector3f::Printerr() const
{
	std::cerr << m_elements[0] << " " << m_elements[1] << " " <<  m_elements[2];
}
